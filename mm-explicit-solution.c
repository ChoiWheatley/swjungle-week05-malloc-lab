...
team_t team = {"Memory Allocator", "HARSH TRIVEDI    ", "201201042",
               "KARAN TANKSHALI  ", "201201043"};
...
/* bp is address of the free block. Since minimum Block size is 16 bytes,
   we utilize to store the address of previous block pointer and next block
   pointer.
*/
...
int mm_init(void) {
  ...
  if ((heap_listp = mem_sbrk(8 * WSIZE)) == NULL) return -1;
 ...
 *   Further if requested size is greater than the current block size at pointer
 * bp and we have the next block as empty with sum of current block size and
 * next block (which happens to be empty) then we dont need call malloc but just
 * combine current block and next block to resize them so as to satisfy the
 * requested realloc size. If nothing can be done then a new block is allocated
 * (using malloc) and the contents of the old block "ptr" are copied to that new
 * block.  Returns the address of this new block if the allocation was
 ...
 *   Removes and inserts appropiate free block pointers to the explicit free
 * list Returns the address of the coalesced block.
 */
static void *coalesce(void *bp) {
  // if previous block is allocated or its size is zero then PREV_ALLOC will be
  // set.
  size_t NEXT_ALLOC = GET_ALLOC(HDRP(NEXT_BLK(bp)));
